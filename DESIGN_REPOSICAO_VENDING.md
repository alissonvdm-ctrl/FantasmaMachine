...
**Amendment (2026-09-09, 2ª e 3ª execuções reais):** Corrigido o host, a chamada seguinte travou clicando na paginação de `/produtos` — o VendPago mostra um modal de "novidades" ao entrar na área logada, cujo backdrop intercepta cliques em qualquer elemento por baixo. `fecharModalDeNovidadesSeAberto` (scraper.ts) remove o modal via `page.evaluate` (mutação de DOM local, sem rede) antes de interagir com a paginação — um clique real no botão "Fechar" do modal poderia disparar algum POST (ex.: marcar notificação como lida) que o `ReadOnlyGuard` bloquearia. A execução seguinte (já sem o modal no caminho) recebeu um timeout de gateway da própria Vercel: `Vercel Runtime Timeout Error: Task timed out after 60 seconds`, mesmo com `maxDuration = 60` (o máximo do plano Hobby). Causa raiz: `waitUntil: "networkidle"` em cada navegação espera a rede ficar ociosa por 500ms — em painéis com widgets/polling em segundo plano isso pode nunca acontecer, e `page.setDefaultTimeout(45000)` deixa cada uma dessas esperas consumir até 45s sozinha. Trocado por `waitUntil: "domcontentloaded"` combinado com `page.waitForSelector` no elemento real que cada etapa precisa (span de paginação de `/produtos`, `table` do relatório de estoque) — mais rápido e igualmente correto, já que não depende de TODA atividade de rede da página cessar. `coletarDadosVendPago` passou a logar o tempo decorrido em cada etapa (`sync.etapa`) para diagnosticar mais rápido se um timeout futuro se repetir.
